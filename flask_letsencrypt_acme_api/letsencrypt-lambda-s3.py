...
def saving_certs_and_key(DOMAIN, bucket_name, pkey_pem, fullchain):
    ...
    path_certificate = (
        "https://s3.ap-south-1.amazonaws.com/" + bucket_name + "/" + certificate_path
    )
    path_private_key = (
        "https://s3.ap-south-1.amazonaws.com/" + bucket_name + "/" + private_key_path
    )
...
def cert():
    ...
    key_authorization = (
        jose.b64encode(token).decode() + "." + jose.b64encode(thumbprint).decode()
    )

    # Computing the SHA-256 digest of the key authorization
    validation = jose.b64encode(
    ...
    Private_key, Certificate = saving_certs_and_key(
        DOMAIN, bucket_name, pkey_pem, finalized_orderr.fullchain_pem
    )
	...
	Certificate path is: {}
	""".format(
        domain_name, email_id, dns_provider, Private_key, Certificate
    )
